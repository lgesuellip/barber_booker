...
from twilio.rest.content.v1.content import ContentInstance, ContentList
...
class WhatsAppAgentTwilio(WhatsAppAgent):
    ...
    def send_carousel(self, to: str) -> None:
        ...
        try:
            contents = self.client.content.v1.contents
            payload = ContentList.ContentCreateRequest(
                {
                    "friendly_name": "barber_booking",
                    "language": "en",
                    "types": ContentList.Types(
                        {
                            "twilio_carousel": ContentList.TwilioCarousel(
                                {
                                    "cards": [
                                        ContentList.CarouselCard(
                                            {
                                                "title": "Book your appointment",
                                                "body": "Choose your slot on our website",
                                                "actions": [
                                                    ContentList.CarouselAction(
                                                        {
            ...
            content = contents.create(payload)
